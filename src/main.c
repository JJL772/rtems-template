/** Contains some code from EPICS base, namely the ne2kpci bug fix code */
/** The intent here is to provide a simple template for writing small RTEMS-based tests.
  * A real application will be configured differently. This is just for quick tests */
#include <rtems.h>
#include <sys/types.h>
#include <bsp.h>
...
POSIX_Init(void* arg)
{
  ...
  if (S_ISCHR(st.st_mode)) {
    /** Configue termios settings for stdin */
    struct termios t;
...
#define CONFIGURE_EXTRA_TASK_STACKS (8192 * RTEMS_MINIMUM_STACK_SIZE)
...
/** Configuring max semaphores causes a hang for some reason */
